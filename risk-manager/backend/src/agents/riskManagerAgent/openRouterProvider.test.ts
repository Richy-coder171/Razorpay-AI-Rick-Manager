/**
 * OpenRouterProvider unit tests — verify the request the provider sends
 * (auth header, model, messages, timeout) against the documented OpenAI-
 * compatible /api/v1/chat/completions contract, WITHOUT any real key.
...
describe('OpenRouterProvider', () => {
  ...
  it('throws on non-2xx so the agent falls back to the mock', async () => {
    stubFetch({ status: 401, body: { error: { message: 'invalid key' } } });
    const provider = new OpenRouterProvider();
